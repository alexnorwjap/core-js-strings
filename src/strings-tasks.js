...
function removeFirstOccurrences(string, value) {
  const valueStart = string.indexOf(value);
  if (valueStart < 0) return string;
  const valueEnd = valueStart + value.length;
  return string.slice(0, valueStart) + string.slice(valueEnd);
}
...
function removeLastOccurrences(string, value) {
  const valueStart = string.lastIndexOf(value);
  if (valueStart < 0) return string;
  const valueEnd = valueStart + value.length;
  return string.slice(0, valueStart) + string.slice(valueEnd);
}
...
function countVowels(string) {
  ...
  let count = 0;
  if (string.length) {
    vowels = string.match(/[aeiouyAEIOUY]/gi);
...
function isPalindrome(string) {
  const clearString = string.toLowerCase().replace(/[^\wа-яА-Я]/g, '');
  const stringMiddle = Math.floor(clearString.length / 2);
  for (let i = 0; i < stringMiddle; i += 1) {
...
function invertCase(string) {
  ...
  const conclusion = string.replace(/[\wа-яА-Я]/gi, callBack);
  return conclusion;
}
...
function extractNameFromTemplate(string) {
  return string.replace(/^Hello, |\s*!$/g, '');
}
...
function getCardId(string) {
  const suit = string.slice(-1);
  const card = string.slice(0, 1);
  const baseSuit = '♣♦♥♠';
  const baseCard = 'A234567891JQK';
  return baseSuit.indexOf(suit) * 13 + baseCard.indexOf(card);
}
